        ...
        if (salesData) {
            ...
            salesData.low_stock_list.forEach(item => {
                lsHTML += `<tr>
                    ...
                    <td>${item.name}</td>
                    <td><span class="uk-label-mix">UK</span> <span style="color:#1e293b; font-weight:800; font-size:1rem;">${item.size}</span></td>                    <td class="text-loss" style="font-weight:bold">${item.stock} left</td>
                </tr>`;
            });
...
async function loadOverviewTable() {
    ...
    productsCache.forEach(p => {
        let sizeHTML = '';
        p.sizes.forEach(s => {
            sizeHTML += `<span style="background:#f1f5f9; padding:0.35rem 0.6rem; border-radius:6px; font-size:0.9rem; font-weight:600; color:#1e293b; margin-bottom:0.25rem;"><span class="uk-label-mix">UK</span> <span style="color:#0f172a; font-weight:900;">${s.size}</span> : <span class="${s.stock<3?'text-loss':'text-primary'}" style="${s.stock>=3?'color:#2563eb !important':''}">${s.stock}</span></span>`;
        });
...
async function searchSellProduct() {
    ...
    prod.sizes.forEach(s => {
        ...
        span.innerHTML = `<span class="uk-label-mix">UK</span> <span style="color:#0f172a; font-weight:900; font-size:1.1rem;">${s.size}</span> <span style="color:#2563eb; font-weight:800; font-size:0.85rem; margin-left:0.4rem;">(${s.stock} left)</span>`;
        span.onclick = () => {
        ...
        row.innerHTML = `
            <div style="display:flex; align-items:center; gap:1rem;">
                <label class="size-return-badge" data-sizeid="${s.id}" title="Click to mark as Return">
                    ...
                    <span class="size-badge-num">R | <span class="uk-label-mix">UK</span>: <span style="color:#0f172a; font-weight:900;">${s.size}</span></span>
                </label>
...
async function loadSizeHeatmap() {
    ...
    res.data.forEach(item => {
        ...
        <div class="heatmap-box ${hClass}">
            <div class="sz"><span class="uk-label-mix">UK</span> <span style="color:#0f172a; font-weight:900; font-size:1.1rem;">${item.size}</span></div>
